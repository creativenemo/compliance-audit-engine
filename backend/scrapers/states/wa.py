"""
Washington Secretary of State — Sprint 4

SOS URL: https://www.sos.wa.gov/corps/search.aspx
Search by entity name → extract status, registered agent, annual report due.

Annual report due: end of anniversary month.
Foreign qualification filing costs: ~$180.
"""
...
def _parse_status(text: str) -> str:
    ...
    if "dissolv" in text_lower or "cancel" in text_lower or "revok" in text_lower:
        return "Dissolved"
    if "delinquent" in text_lower or "administratively" in text_lower:
        return "Delinquent"
    return text.strip() or "Unknown"


class WashingtonScraper(BaseSosScraper):
    state_code = "WA"
    state_name = "Washington"
    sos_url = "https://www.sos.wa.gov/corps/search.aspx"
        ...
        async with async_playwright() as pw:
            ...
            try:
                ...
                name_input = page.locator(
                    "input[name*='BusinessName'], input[id*='BusinessName'], "
                ...
                links = page.locator("table td a, .results a, a[href*='UBI']")
                ...
                # Entity name
                for sel in ["h1", "h2", ".entityName", "#BusinessName", "strong"]:
                ...
                # Formation date
                m = re.search(
                    r"(?:Date of Formation|Formation Date|Filing Date|Date Filed)[:\s]+([\d/]+)",
                    body_text,
                )
                if m:
                ...
                # Annual report due
                m = re.search(
                    r"(?:Annual Report Due|Renewal Date|Expiration Date)[:\s]+([^\n]+)",
                    body_text,
                )
                if m:
    ...
    async def scrape_entity(self, legal_name: str) -> EntityRegistration:
        ...
        return EntityRegistration(
            ...
            annual_report_due=raw.get("annual_report_due", "End of anniversary month"),
            raw_data=raw,
        )
