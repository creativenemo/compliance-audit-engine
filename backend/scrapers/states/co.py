"""
Colorado Secretary of State — Sprint 4

SOS URL: https://www.sos.state.co.us/biz/BusinessEntityCriteriaExt.do
Search by name → results → click first match → extract entity details.

Annual report due: within 2 months of anniversary month.
Foreign qualification filing costs: ~$100.
"""
...
def _parse_status(text: str) -> str:
    ...
    if "good standing" in text_lower or "active" in text_lower:
        return "Active"
    if "dissolv" in text_lower or "deactivat" in text_lower or "revok" in text_lower:
        return "Dissolved"
...
class ColoradoScraper(BaseSosScraper):
    state_code = "CO"
    state_name = "Colorado"
        ...
        async with async_playwright() as pw:
            ...
            try:
                ...
                name_input = page.locator(
                    "input[name='masterFileId'], input[name='entityName'], "
                    "input[id*='entityName'], input[id*='name'], input[type='text']"
                ).first
                ...
                links = page.locator("table td a, .results a")
                ...
                # Formation date
                m = re.search(
                    r"(?:Formation Date|Formed|Date of Formation|File Date)[:\s]+([\d/]+)",
                    body_text,
                )
                if m:
                ...
                # Periodic report due
                m = re.search(
                    r"(?:Periodic Report Due|Annual Report Due|Report Due)[:\s]+([^\n]+)",
                    body_text,
                )
                if m:
                    raw["periodic_report_due"] = m.group(1).strip()
    ...
    async def scrape_entity(self, legal_name: str) -> EntityRegistration:
        ...
        annual_due = raw.get("periodic_report_due", "Within 2 months of anniversary month")
